...
filesystem_path = "filesystem4.json"
current_location = "/"
current_user = None
filesystem = {
    "Contents": {
...
def show_help():
    ...
    if current_user["type"] == "admin":
        ...
        print("ops <username> <type> - Show operations log")
...
def check_permission(filename, type_):
    ...
    if attrs[type_] == "user":
        ...
        if current_user["type"] == "admin":
            ...
            filesystem["Contents"]["Admin Log"]["Contents"]["Operations"].append({
                "user": current_user["username"],
                "action": f"Denied access to {current_location+'/'+filename}",
                "type": "dangerous",
                "time": time.time()
            })
            return False
...
def read_operations(args):
    args = args.split(" ")
    if len(args) != 2:
        print("Wrong arguments")
        return
    username, type_ = args
    if type_ not in ("all", "dangerous", "casual"):
        print("Second argument must be 'all', 'dangerous' or 'casual'")
    for user in filesystem["Contents"]["Admin Log"]["Contents"]["Registrations"]:
        if user["username"] == username:
            break
    else:
    ...
    for record in filesystem["Contents"]["Admin Log"]["Contents"]["Operations"]:
        if record["user"] == username:
            if type_ == "all" or record["type"] == type_:
                print(record["user"], record["type"], record["action"], "time:", record["time"])
...
def loop():
    ...
    while True:
        ...
        if current_user["type"] != "admin":
            filesystem["Contents"]["Admin Log"]["Contents"]["Operations"].append({
                "user": current_user["username"],
                "action": f"{current_location}: {choice}",
                "type": "casual",
                "time": time.time()
            })
        if time.time() - start > 300 and current_user["type"] == "user":
        ...
        if choice == "pwd":
            ...
            unregister(choice[6:])
        elif choice.startswith("ops ") and current_user["type"] == "admin":
            read_operations(choice[4:])
